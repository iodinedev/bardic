...
export function init() {
  router.get("/GetNfts/:projectID", async (ctx) => {
    ...
    const api_url =
      "https://api.nft-maker.io/GetNfts/" +
      process.env.API_KEY +
      "/" +
      projectID +
      "/all";
  ...
  router.get("/GetRarityRanks", async (ctx) => {
    const api_url =
      "https://cnft.tools/api/rankings/8dd5717e7d4d993019dbd788c19837910e3fcf647ab282f828c80a7a/";
  ...
  router.get("/AssetsFromBytes/:address", async (ctx) => {
    const address = ctx.params.address;
    const byteArray: Uint8Array = toByteArray(address);
    const assets: any[] = [];

    const value = CSL.Address.from_bytes(byteArray);

    const api_url =
      "https://cardano-mainnet.blockfrost.io/api/v0/addresses/" + value.to_bech32();
    const headers = { project_id: "cAiR6OBMC8HcwEc8kf9BVPR05eqW1Cx0" };
    ...
    try {
      ...
      if (response.status === 200) {
        assets.push(response.data.amount);
      }
    } catch(err) {}

    ctx.body = {address: value.to_bech32(), assets: assets};
  })
}

function toByteArray(hexString): Uint8Array {
  var result: number[] = [];
