...
function init(
  ...
  if (moonsArray[0] != "No") {
    ...
    for (let moon of moonsArray) {
      if (moon == "small moon") {
        ...
        var size = (0.1 * planetRadius) / planetSize;
      ...
      const moonMaterial = new THREE.MeshPhongMaterial({
        color: color,
        map: loader.load("moon" + i + ".png"),
        bumpMap: loader.load("moon" + i + ".png"),
        bumpScale: bumpScale,
        displacementMap: loader.load("moon" + i + ".png"),
        displacementScale: 0,
        reflectivity: 1,
        shininess: 0,
      });
  ...
  if (
    atmosphere == "Red laser shield" ||
    atmosphere == "Purple laser shield" ||
    atmosphere == "Green laser shield"
  ) {
...
function runAnimation(seedDate, rings, raritiesArray) {
  ...
  function animate() {
    ...
    if (raritiesArray.includes("Versicoloria colony")) {
      ...
      var lastDigit =
        0.3 + 0.9 * Math.abs(Math.sin((time % rate) / rate - 0.5));
